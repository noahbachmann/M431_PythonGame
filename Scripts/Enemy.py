...
class BasicShooter(Enemy):
    ...
    def shoot(self):
        ...
        Shot(spawnPos,self.damage,450,1,self.angle,Enemy_Laser.ENEMY_LASER, self.groups(),700, self.offset.copy(), (7,14), Enemy_Laser.animationArray_laser_explosion)
...
class DoubleShooter(BasicShooter):
    ...
    def shoot(self):
        ...
        offset = pygame.math.Vector2(math.cos(math.radians(self.angle)), math.sin(math.radians(self.angle))) * self.rect.height / 2
        if not self.swap:
            Shot(self.rect.center + offset - sideOffset,self.damage,450,1,self.angle,Enemy_Laser.ENEMY_LASER, self.groups(),700, self.offset.copy(), (7,14), Enemy_Laser.animationArray_laser_explosion)
            Shot(self.rect.center + offset + sideOffset,self.damage,450,1,self.angle,Enemy_Laser.ENEMY_LASER, self.groups(),700, self.offset.copy(), (7,14), Enemy_Laser.animationArray_laser_explosion)
        else:
            if self.right:
                Shot(self.rect.center + offset + sideOffset,self.damage,450,1,self.angle,Enemy_Laser.ENEMY_LASER, self.groups(),700, self.offset.copy(), (7,14), Enemy_Laser.animationArray_laser_explosion)
